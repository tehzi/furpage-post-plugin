import React, { Component, ComponentClass, ComponentType, ReactNode, SyntheticEvent } from "react";
import { isElement, last } from "lodash";
import { compose } from "redux";
import { connect, InferableComponentEnhancerWithProps, MapDispatchToProps, MapStateToProps } from "react-redux";
...
const mapStateToProperties: MapStateToProps<SiteFromStoreProps, OwnSiteProps, AppStore> = (store) => {
    let userId = null;
    const { updateUrl: { url = null } = {}, images: { [window.location.href]: image = null } = {} } = store;
...
const mapDispatchToProperties: MapDispatchToProps<DispatchSiteProps, OwnSiteProps> = (dispatch) => ({
    onStartAuth(): void {
        dispatch(vkAuth());
    },
    onAddImage(event, image, tagSelector): void {
        let tags: string[] = [];
        if (tagSelector) {
            tags = [...(document.querySelectorAll(tagSelector) as unknown as Iterable<Element>)].map(
    ...
    return compose(
        ...
        (Site: ComponentType<SiteProps & { mount: Element }>) =>
            class extends Component<SiteProps> {
                ...
                static displayName = `withSite(${(Component as ComponentClass).displayName})`;
                ...
                shouldComponentUpdate(nextProperties: SiteProps): boolean {
                    ...
                    if (isUpdate) {
                        this.image = last([
                            ...(document.querySelectorAll(selector) as unknown as Iterable<HTMLImageElement>),
                        ]);
                        const isElementAppended = isElement(document.querySelector(`#${pluginMount}`));
                ...
                insertRoot = (): void => {
                    const { parentNode, parentNode: { insertBefore = (): void => {} } = {} } = this.image;
                    const isElementAppended = isElement(document.querySelector(`#${pluginMount}`));
                    if (!isElementAppended) {
                        ...
                        >
                            <Button
                                ...
                                onClick={(event: SyntheticEvent): void => {
                                    ...
                                    if (canAdd(image)) {
                                        onAddImage(event, this.image.src, tagSelector);
                                    }
                                }}
                            >
                                <ButtonState />
